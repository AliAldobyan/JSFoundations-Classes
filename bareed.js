...
class Customer extends Person{
  ...
  _isInRange = (vendor) => {
    if (this.location.distanceTo(vendor.location) <= vendor.range){
      ...
      return false;
      console.log(`The customer ${this.name} is not the range of ${vendor.name}`)
    }
  }
  ...
  _haveEnoughMoney = (vendor, numberOfIceCreams) => {
    if (this.wallet.money >= (vendor.price * numberOfIceCreams)){
      ...
      return false;
      console.log(`The customer ${this.name} has ${this.wallet.credit} and it is not enough to buy ${numberOfIceCreams} it costs ${vendor.price * numberOfIceCreams}`)
    }
  }
